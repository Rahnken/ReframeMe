...
export const goalsQueryOptions = (token: string) =>
  queryOptions({
    queryKey: ["goals", token],
    queryFn: () => getAllGoalsQuery(token),
  });
export const goalQueryIdOptions = (userToken: string, goalId: string) =>
  queryOptions({
  ...
  return useMutation({
    ...
    onSuccess: (data) => {
      ...
      queryClient.invalidateQueries({
        queryKey: ["groups", token, data.sharedGoals.group_id],
      });
      onSuccessCallback();
    },
    onError: (error) => {
...
export const useUpdateGoalProgressMutation = (
  token: string,
  onSuccessCallback: () => void,
  onErrorCallback: (error: Error) => void
) => {
  return useMutation({
    ...
    mutationFn: (updateRequestBody: GoalProgressUpdateBody) =>
      updateGoalProgressById(token, updateRequestBody),
    onSuccess: () => {
      onSuccessCallback();
